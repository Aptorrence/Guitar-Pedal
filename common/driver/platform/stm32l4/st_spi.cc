...
namespace driver
{
    namespace Stml4
    {
        ...
        bool HwSpi::Write(std::span<uint8_t> tx_data)
        {
            ...
            for (size_t i = 0; i < tx_data.size(); i++)
            {
                ...
                *(volatile uint8_t *)&instance->DR = tx_data[i];
                 ...
                 * Even if only writing, RXNE flag has to be cleared before proceeding to the next
                 * byte write.
                 */
                while (!(instance->SR & SPI_SR_RXNE))
                {
                 ...
                 * Dereferencing the data reg performs a read. Type casting to void explicitly
                 * discards the unused value.
                 */
        ...
        bool HwSpi::Init()
        {
            // TODO: Runtime validation of enum values (will change to compile time checks in the
            // future and maybe make a private function for these checks)
